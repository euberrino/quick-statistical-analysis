#' Quick statistical analysis for small - medium datasets.
#'
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
    menuItem("Uploading Dataset", tabName = "upload", icon = icon("upload")),
    menuItem("Preprocessing", tabName = "preprocessing", icon = icon("search")),
    menuItem("Normality assesment", tabName = "normal", icon = icon("poll")),
      ...
      tabItem(tabName = "upload",
              fluidRow(
                ...
                mainPanel(width = 12,
                  ...
                  tabPanel("Missing Values",DT::dataTableOutput("missing")))
                  # uiOutput("out")))
